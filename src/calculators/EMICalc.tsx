...
export default function EMICalc({ color, t, onResult }: CalcProps) {
  ...
  // ─── PDF export (Blob download — works on mobile) ───
  ...
  return (<div>
    {/* Mode toggle */}
    <div style={tabRow}>
      <button onClick={() => { setCompareMode("calc") }} style={tabStyle(compareMode === "calc", color, t)}>EMI</button>
      <button onClick={() => { setCompareMode("compare") }} style={tabStyle(compareMode === "compare", color, t)}>Compare</button>
      <button onClick={() => { setCompareMode("prepay") }} style={tabStyle(compareMode === "prepay", color, t)}>Prepay</button>
    </div>

    {compareMode === "calc" ? (<div>
      {/* Hero result */}
      <HeroNumber aria-live="polite" label="Monthly EMI" value={currency(emi)} color={color} />
      ...
      <div style={tabRowSm}>
        {FINANCE.LOAN_PRESETS.map(lp => (
          <button key={lp.id} onClick={() => { setRate(lp.rate); setN(lp.tenure) }}
      ...
      <div style={{ background: total <= compare.total2 ? `${tokens.color.success}10` : `${tokens.color.danger}10`,
        ...
        <HeroNumber aria-live="polite" value={currency(compare.saved)} color={total <= compare.total2 ? tokens.color.success : tokens.color.danger} style={{ padding: 0 }} />
      ...
      {pp.monthsSaved > 0 && (
        <div style={{ background: `${tokens.color.success}10`, border: `1px solid ${tokens.color.success}25`,
          ...
          <HeroNumber aria-live="polite" value={`${pp.monthsSaved} months early`} color={tokens.color.success} style={{ padding: `${tokens.space.sm}px 0` }} />
